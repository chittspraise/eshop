...
CREATE UNIQUE INDEX orders_pkey ON public."order" USING btree (id);

CREATE UNIQUE INDEX orders_slug_key ON public."order" USING btree (slug);
...
alter table "public"."order" add constraint "orders_slug_key" UNIQUE using index "orders_slug_key";

alter table "public"."order" add constraint "orders_user_fkey" FOREIGN KEY ("user") REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;

alter table "public"."order" validate constraint "orders_user_fkey";
...
BEGIN
    IF NEW.raw_user_meta_data->>'avatar_url' IS NULL OR NEW.raw_user_meta_data->>'avatar_url' = '' THEN
        NEW.raw_user_meta_data = jsonb_set(
            ...
            '{avatar_url}',
            '"https://as2.ftcdn.net/v2/jpg/05/89/93/27/1000_F_589932782_vQAEAZhHnq1QCGu5ikwrYaQD0Mmurm0N.jpg"'::jsonb
        );
    END IF;
...
-- on authuesr, create an new user in the
CREATE
...
create policy "Allow operations for all auth users"
