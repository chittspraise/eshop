...
CREATE POLICY "Allow operations for all auth users" ON "public"."order_item" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "Enable delete for admin users only" ON "public"."product" FOR DELETE TO "authenticated" USING ((( SELECT "users"."type"
   FROM "public"."users"
  WHERE ("users"."id" = "auth"."uid"())) = 'ADMIN'::"text"));
...
CREATE POLICY "Enable update for auth users" ON "public"."users" FOR UPDATE TO "authenticated" USING (true);
